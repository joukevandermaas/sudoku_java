...
public class ForcingChainsStrategy implements Strategy {
	...
	public boolean removePossibilities(Sudoku puzzle) throws SudokuException,
		...
		// Only do doubles (cells with two possibilities) to avoid having too
		// many chains.
		...
		for (Cell c : doubles) {
			...
			if (chainResult != null) {
				...
				Cell chainResultEquiv = puzzle.getCell(chainResult.getRow(),
						chainResult.getColumn());
				currentLevel--;
				return chainResultEquiv.removeOtherPossibilities(chainResult
						.getValue());
			}
		}
	...
	private Cell finishChain(int[][] copy, Cell c) throws SudokuException,
			InvalidSudokuException {
	...
	private Cell findNewOverlap(Sudoku pos1, Sudoku pos2, int[][] copy)
		...
		for (int row = 0; row < Sudoku.SUDOKU_SIZE; row++) {
			for (int col = 0; col < Sudoku.SUDOKU_SIZE; col++) {
				...
				if (copy[row][col] == 0 && c1.hasValue() && c2.hasValue()
